...
export function showLatestRun(element, stats) {
    ...
    const html = [
        ...
        `  <div>Average: ${(workflow_avg.total_time / 1000 || 0).toFixed(2)}s \u00b1${(workflow_avg.std_time / 1000 || 0).toFixed(2)}s (${workflow_avg.count || 0} runs)</div>`,
        `  <div>VRAM Peak: ${(profile.totalVramPeak / 1e9).toFixed(2)}GB (avg \u00b1${(workflow_avg.std_vram / 1e9 || 0).toFixed(2)}GB)</div>`,
        `  <div>RAM Peak: ${(profile.totalRamPeak / 1e9).toFixed(2)}GB (avg \u00b1${(workflow_avg.std_ram / 1e9 || 0).toFixed(2)}GB)</div>`,
