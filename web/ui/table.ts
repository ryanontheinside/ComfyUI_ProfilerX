...
export function applyTableStyles(table: HTMLTableElement) {
    table.style.cssText = `
        width: 100%;
    ...
    table.querySelectorAll('thead th').forEach(th => {
        const header = th as HTMLTableHeaderCellElement;
...
export function makeSortable(table: HTMLTableElement) {
    const headers = table.querySelectorAll('th');
    const tbody = table.querySelector('tbody') as HTMLTableSectionElement;
    
    headers.forEach((header, index) => {
        const th = header as HTMLTableHeaderCellElement;
        ...
        th.addEventListener('click', () => {
            ...
            headers.forEach(h => {
                const header = h as HTMLTableHeaderCellElement;
            ...
            // Sort rows
            rows.sort((a, b) => {
                const aCell = a.children[index] as HTMLTableCellElement;
                const bCell = b.children[index] as HTMLTableCellElement;
                ...
                if (th.classList.contains('sort-numeric')) {
                    ...
                    const convertToBaseUnit = (text: string): number => {
                        const num = parseFloat(text.replace(/[^\d.-]/g, '')) || 0;
                        if (text.includes('GB')) return num * 1024 * 1024 * 1024;
                        if (text.includes('MB')) return num * 1024 * 1024;
                        if (text.includes('KB')) return num * 1024;
                        if (text.includes('s') && !text.includes('ms')) return num * 1000;
                        return num;
                    };
                    ...
                    const parseTime = (cell: HTMLTableCellElement): number => {
                    ...
                    const getMainContent = (cell: HTMLTableCellElement): string => {
